...
class YOLODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if need_fill > 0:
            # 从全局类别中排除已存在的，随机采样填充
            candidate_names = [n for n in self.global_class_names if n not in real_name_set]
            fill_names = random.sample(candidate_names, need_fill)
        ...
        # 5. 修正框坐标
        if raw_boxes:
            boxes = self.correct_boxes(raw_boxes, scale, paste_x, paste_y, w, h)
        else:
            boxes = []


        img = self.resize_and_pad(Image.open(img_path).convert("RGB"), target_size=self.input_size)

        return img, boxes, final_unique_names